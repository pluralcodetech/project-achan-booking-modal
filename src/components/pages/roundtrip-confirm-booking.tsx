...
@Component({
    tag: 'roundtrip-confirm-booking',
    styleUrl: 'pages.css',
    shadow: true
})

export class RoundtripConfirmBooking {
  ...
  callConfirmBookingApi = async () => {
    ...
    ConfirmBooking.append('returndate', this.localState?.returnPickupDate);
    ConfirmBooking.append('returntime', this.localState?.returnPickupTime);
    ConfirmBooking.append('estimated_returnarivaltime', this.localState?.returnArrivalTime);
    
    try {
        const response = await fetch(`https://watchoutachan.herokuapp.com/api/fourth`,
          {
            method: 'post',
            body: ConfirmBooking,
          }
      ...
      handleErrors(response);
        ...
        localStorage.setItem("roundtripCBTD", JSON.stringify(json));
    ...
    render() {
        return (
            ...
            <div>
                <modal-booking-details
                    date={this.localState?.returnPickupDate} //date
                    time={this.localState?.returnPickupTime} //time
                    airport={ this.estimateState?.third_cost?.from}
                    destinationAddress={this.estimateState?.third_cost?.to}
                    estimatedPriceMax={this.estimateState?.third_cost?.est_max}
                    estimatedPriceMin={this.estimateState?.third_cost?.est_min}
                ></modal-booking-details>
            </div>
              <div>
                <modal-booking-details
                    // returnproperties={true}
                    date={this.localState?.returnPickupDate} //date
                    time={this.localState?.returnArrivalTime} //time
                    airport={ this.estimateState?.second_cost?.from}
                    destinationAddress={this.estimateState?.fourth_cost?.to}
                    estimatedPriceMax={this.estimateState?.fourth_cost?.est_max}
                    estimatedPriceMin={this.estimateState?.fourth_cost?.est_min}
                ></modal-booking-details>
              </div>
